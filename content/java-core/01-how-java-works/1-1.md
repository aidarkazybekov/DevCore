...
## How It Works
The **Class Loader** reads `.class` files and places class metadata into Metaspace (native memory outside the heap). The **Runtime Data Areas** divide memory into two categories: shared areas (Heap for all object instances; Metaspace for class metadata, static fields, and bytecode) and per-thread areas (JVM Stack of method frames, PC Register for the current instruction address, Native Method Stack for JNI calls). The **Execution Engine** starts as an interpreter and promotes hot methods through JIT tiers: C1 (Tiers 1–3, fast compilation) then C2 (Tier 4, aggressive optimizations — inlining, escape analysis, loop unrolling). OSR (On-Stack Replacement) lets the JVM compile a method while it is already executing. If an optimistic assumption is later invalidated (e.g. a new subclass appears), the JVM deoptimizes and falls back to the interpreter.

## Gotcha
`-Xmx` controls **only** the heap. Real process RSS also includes Metaspace (unbounded unless `-XX:MaxMetaspaceSize` is set), JIT CodeCache (default 240 MB), thread stacks (N threads × `-Xss`), direct buffers, and GC bookkeeping. Expect RSS ≈ 1.5–2× your `-Xmx`. Also: a `static` field lives in Metaspace (shared), so it is not thread-safe just because it is static.

## Recap
- **Class Loader**: `.class` → metadata in Metaspace (load → link → initialize).
- **Heap** (shared): all object instances and arrays, bounded by `-Xmx`.
- **Metaspace** (shared): class metadata, `static` fields — native memory, unbounded by default.
