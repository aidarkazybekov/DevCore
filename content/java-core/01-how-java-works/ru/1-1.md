...
## TL;DR
JVM загружает байт-код в разные области памяти (Heap, Metaspace, JVM Stack) и исполняет его сначала через интерпретатор, затем компилирует горячие пути в нативный код через JIT (C1 → C2). `-Xmx` ограничивает только heap — реальная память процесса в 1.5–2× больше.

## Analogy
Представьте JVM как завод. Погрузочная платформа (Class Loader) получает сырьё (`.class`-файлы) и хранит чертежи на складе (Metaspace). Каждый рабочий (поток) имеет собственный верстак (JVM Stack). Готовая продукция накапливается в общем хранилище (Heap). Мастер (Execution Engine) сначала читает инструкции вручную (интерпретатор), но когда задача повторяется достаточно часто — автоматизирует её с помощью машины (JIT-компилятор).

## What & Why
Обещание Java «write once, run anywhere» целиком держится на абстракции JVM: компилятор Java создаёт платформо-нейтральный байт-код, а JVM транслирует его в нативные инструкции во время выполнения. Поскольку heap, Metaspace и стеки потоков — это отдельные бюджеты памяти, разработчики, следящие только за `-Xmx`, удивляются, когда `top` показывает RSS намного больше настроенного heap. Каждый Java-инженер должен понимать структуру памяти, чтобы диагностировать `OutOfMemoryError`, настраивать GC и писать потокобезопасный код.

## How It Works
**Class Loader** читает `.class`-файлы и помещает метаданные классов в Metaspace (native-память вне heap). **Runtime Data Areas** делят память на два типа: общие области (Heap для всех объектов; Metaspace для метаданных классов и байт-кода; значения static-полей живут в Heap, внутри объекта `java.lang.Class` каждого класса) и per-thread области (JVM Stack из фреймов методов, PC Register с адресом текущей инструкции, Native Method Stack для JNI-вызовов). **Execution Engine** начинает работу как интерпретатор и продвигает горячие методы через тиры JIT: C1 (Tiers 1–3, быстрая компиляция), затем C2 (Tier 4, агрессивные оптимизации — inlining, escape analysis, loop unrolling). OSR (On-Stack Replacement) позволяет JVM компилировать метод прямо во время его выполнения. Если оптимистическое предположение нарушается (например, появляется новый подкласс), JVM деоптимизирует и откатывается к интерпретатору.

## Gotcha
`-Xmx` контролирует **только** heap. Реальный RSS процесса включает также Metaspace (не ограничен без `-XX:MaxMetaspaceSize`), JIT CodeCache (по умолчанию 240 MB), стеки потоков (N потоков × `-Xss`), direct-буферы и структуры GC. Ожидайте RSS ≈ 1.5–2× от `-Xmx`. Также: значение `static`-поля живёт в Heap (в объекте `java.lang.Class`), общем для всех потоков, поэтому оно не является thread-safe только потому, что оно статическое.

## Recap
- **Class Loader**: `.class` → метаданные в Metaspace (load → link → initialize).
- **Heap** (shared): все объекты и массивы, ограничен `-Xmx`.
- **Metaspace** (shared, native-память): метаданные классов + байт-код. Значения static-полей хранятся в Heap (объект `java.lang.Class`).
- **JVM Stack** (per-thread): фреймы с локальными переменными и operand stack.
- **Execution Engine**: интерпретатор → C1 JIT → C2 JIT; прогрев занимает секунды и минуты.
- Всегда задавайте `-XX:MaxMetaspaceSize` в production.
...
- **Method Area (Metaspace с Java 8)** — метаданные классов, Runtime Constant Pool, байт-код методов. Находится в native-памяти, не ограничена `-Xmx`. Значения static-полей хранятся в Heap (в объекте `java.lang.Class`).
...
> [!gotcha]
> Локальная переменная-ссылка живёт в стеке, но сам объект, на который она указывает, — в куче. Поэтому `static`-поле — НЕ thread-safe автоматически: его значение живёт в Heap (в объекте `java.lang.Class`, shared), и все потоки пишут в одну ячейку.
...
• Method Area / Metaspace -- метаданные классов, Runtime Constant Pool, байт-код методов. Значения static-полей живут в Heap (в объекте `java.lang.Class`).
...
Частая ловушка: static поле 'thread-safe потому что static' -- неверно. Значение static-поля живёт в Heap (в объекте `java.lang.Class`, shared), то есть к нему тоже одновременно обращаются все потоки.
...
(1) `counter` -- static-поле → Heap (в объекте `java.lang.Class`; одно на всю JVM).
...
## Checkpoint
### [1-1-cp0] Часто считается, что static-поле thread-safe. Где оно находится в памяти и почему это предположение неверно?
Значение `static`-поля живёт в Heap (в объекте `java.lang.Class`), который является общим для всех потоков. Каждый поток JVM одновременно читает и пишет в один и тот же слот памяти. Без явной синхронизации (`volatile`, `synchronized` или атомарного типа) конкурентные записи создают race condition — поле не является thread-safe, несмотря на то что оно `static`.

### [1-1-cp1] Java-процесс показывает 3.5 GB RSS при `-Xmx2g`. Назовите три области памяти вне heap, которые объясняют разницу.
Metaspace (метаданные классов в native-памяти, не ограничена без `-XX:MaxMetaspaceSize`), JIT CodeCache (`-XX:ReservedCodeCacheSize`, по умолчанию 240 MB) и стеки потоков (количество потоков × `-Xss`, обычно ~1 MB каждый). Дополнительными источниками являются direct-буферы (`-XX:MaxDirectMemorySize`) и структуры GC.

## Key Terms
### class loader
Подсистема JVM, ответственная за загрузку, связывание и инициализацию классов. Она читает `.class`-файлы из classpath и помещает их метаданные в Metaspace, делая класс доступным для создания объектов и выполнения.

### bytecode
Платформо-нейтральные двоичные инструкции, хранящиеся в `.class`-файлах и создаваемые компилятором Java. Execution Engine JVM интерпретирует или JIT-компилирует байт-код в нативные машинные инструкции во время выполнения, обеспечивая переносимость Java.

### heap
Область памяти JVM, общая для всех потоков, где выделяются экземпляры объектов и массивы. Делится на Young Generation (Eden + Survivor spaces) и Old Generation; её размер ограничен флагом JVM `-Xmx`.
